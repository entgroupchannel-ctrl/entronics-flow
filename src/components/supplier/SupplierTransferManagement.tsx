...
import { InternationalTransferForm } from "../international-transfer/InternationalTransferForm";
...
interface SupplierTransferManagementProps {
  suppliers: any[];
}

export function SupplierTransferManagement({ suppliers }: SupplierTransferManagementProps) {
  ...
  const { data: transferData, isLoading, refetch } = useQuery({
    queryKey: ["supplier-transfer-report", startDate, endDate, selectedSupplier, status, currency],
    queryFn: async () => {
      let query = supabase
  ...
  const handleEditTransfer = (request: any) => {
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", `รายงานการโอนเงิน_Supplier_${format(new Date(), "yyyy-MM-dd")}.csv`);
    ...
    <div className="space-y-6">
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <div className="flex justify-between items-center">
          <TabsList>
            <TabsTrigger value="list">รายการโอนเงิน</TabsTrigger>
          ...
          <div className="flex gap-2">
            {activeTab === "list" && (
              <>
                <Button onClick={exportToCSV} disabled={!transferData?.length} variant="outline">
        ...
        <TabsContent value="form" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                การโอนเงินไปยัง Supplier ต่างประเทศ
              </CardDescription>
            </CardHeader>
            <CardContent>
