-- Create supplier grade enum
CREATE TYPE supplier_grade AS ENUM ('A+', 'A', 'B+', 'B', 'C');

-- Create suppliers table (ตารางซัพพลายเออร์หลัก)
CREATE TABLE public.suppliers (
  ...
  -- เกรดและการประเมิน
  supplier_grade supplier_grade DEFAULT 'C',
  quality_score NUMERIC(2,1) DEFAULT 0 CHECK (quality_score >= 0 AND quality_score <= 5),
  price_competitiveness NUMERIC(2,1) DEFAULT 0 CHECK (price_competitiveness >= 0 AND price_competitiveness <= 5),
  reliability_score NUMERIC(2,1) DEFAULT 0 CHECK (reliability_score >= 0 AND reliability_score <= 5),
  
  -- ใบอนุญาต/มาตรฐาน
  business_license TEXT,
  certification_iso BOOLEAN DEFAULT FALSE,
  certification_ce BOOLEAN DEFAULT FALSE,
  certification_fcc BOOLEAN DEFAULT FALSE,
  
  -- หมวดหมู่และความเชี่ยวชาญ
