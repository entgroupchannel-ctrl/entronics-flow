-- Insert sample technicians with correct enum values (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.technicians WHERE email = 'somchai@company.com') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.technicians WHERE email = 'sunisa@company.com') THEN
    INSERT INTO public.technicians (name, email, phone, specialization, is_available, current_workload, rating, total_jobs) VALUES
    ('สุนิษา ทองดี', 'sunisa@company.com', '082-345-6789', 'electrical', true, 0, 4.9, 52);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM public.technicians WHERE email = 'virach@company.com') THEN
    INSERT INTO public.technicians (name, email, phone, specialization, is_available, current_workload, rating, total_jobs) VALUES
